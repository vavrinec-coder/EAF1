...
export type FsMonthlySheetSpec = {
...
export async function createFsMonthlySheet(spec: FsMonthlySheetSpec): Promise<void> {
  await Excel.run(async (context) => {
    const worksheets = context.workbook.worksheets;
    const controlsSheet = worksheets.getItemOrNullObject("Controls");
    controlsSheet.load("name");
    let sheet = worksheets.getItemOrNullObject("FS Monthly");
    sheet.load("name");
    await context.sync();
    ...
    if (sheet.isNullObject) {
      sheet = worksheets.add("FS Monthly");
    } else {
...
function applyFsSectionRow(
  ...
  sheet.getRange(`A${rowNumber}`).values = [[label]];
  const rowRange = sheet.getRangeByIndexes(rowNumber - 1, 0, 1, totalColumns);
  rowRange.format.fill.color = fillColor;
}

function applyColumnWidths(
  sheet: Excel.Worksheet,
  widths: FsMonthlySheetSpec["columnWidths"]
): void {
